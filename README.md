...
- `http-generic-api/openapi.custom-gpt.auth-dispatcher.yaml` — admin single-host OpenAPI spec for `auth.mad4b.com` (v4.0.0-mcp). Five meta-ops: `activateSession` (open session + platform context), `listTools` (discover DB tool registry), `callTool` (execute any registered tool), `writeSessionTurn` (persist conversation turn), `endSession` (close + Drive archive). Tenant GPT uses `http-generic-api/openapi.tenant-gpt.auth.yaml` on the same host with OAuth/user-JWT scoping.
- `http-generic-api/openapi.gpt-action.local-connector.yaml` — local connector OpenAPI spec for `connector.mad4b.com`. Ten ops: health, github, gcloud, shell, files, fetch-upload, shell-fetch-upload, ps (PowerShell), win (Windows UI), n8n.
...
- **Admin Platform** (`openapi.custom-gpt.auth-dispatcher.yaml` -> `auth.mad4b.com`): 5 MCP-style meta-ops (activateSession, listTools, callTool, writeSessionTurn, endSession); all platform tools discovered and called via the registry. Auth: admin/service `BACKEND_API_KEY`.
...
- **Local Connector** (`openapi.gpt-action.local-connector.yaml` -> `connector.mad4b.com`, or `connect.mad4b.com` if configured as the connector host alias): 10-op standalone local execution bridge for break-glass shell/file/GitHub/gcloud/PS/Win/n8n on the admin Windows device.
