...
Optional dev diagnostics connector: `http-generic-api/openapi.gpt-action.dev-diagnostics.yaml` points directly at `https://dev.mad4b.com` for read-only development checks only: `GET /health`, `GET /deployment-info`, and authenticated `GET /dev/db/status`. It is not part of the default two-connector architecture and should be installed only during dev-runtime certification. It must not expose restore, shell, DB mutation, or deployment operations.
