...
-- INSERT IGNORE — safe to re-run; will not overwrite if already configured.
-- After running, retrieve the secret with:
--   SELECT JSON_UNQUOTE(JSON_EXTRACT(config_json, '$.client_secret'))
--     FROM platform_runtime_config
--    WHERE config_key = 'tenant_gpt.oauth.client';
-- Then paste that value into the Custom GPT OAuth authentication panel.
