...
ON DUPLICATE KEY UPDATE display_name=VALUES(display_name), description=VALUES(description), http_method=VALUES(http_method), http_path=VALUES(http_path), path_param_keys=VALUES(path_param_keys), input_schema=VALUES(input_schema), fixed_body=VALUES(fixed_body), tags=VALUES(tags), sort_order=VALUES(sort_order)
'@
...
('connector_files','Device Files','Explore allowlisted device paths, list drives, locate repo roots, and read or write allowlisted files.','POST','/connector/{device_id}/files','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["list","list_drives","locate_repo","read","write"]},"path":{"type":"string"},"content":{"type":"string"},"markers":{"type":"array","items":{"type":"string"}},"max_depth":{"type":"integer","minimum":0,"maximum":8},"max_matches":{"type":"integer","minimum":1,"maximum":50},"max_entries":{"type":"integer","minimum":1,"maximum":500},"user_id":{"type":"string"}}}',NULL,'device,filesystem,repo_discovery',260),
('connector_dependencies','Device Dependencies','Inspect or install allowlisted recovery dependencies on the local device.','POST','/connector/{device_id}/dependencies','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["status","list","install"]},"package_key":{"type":"string","enum":["gh","googlecloudsdk"]},"timeout_ms":{"type":"integer","minimum":1000,"maximum":600000},"user_id":{"type":"string"}}}',NULL,'device,dependency_recovery,state_changing',262),
('connector_apps','Device Apps','List, launch, inspect, or close allowlisted desktop applications on the local device.','POST','/connector/{device_id}/apps','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["status","list","launch","status_app","close"]},"app_alias":{"type":"string"},"timeout_ms":{"type":"integer","minimum":1000,"maximum":120000},"user_id":{"type":"string"}}}',NULL,'device,desktop_app,interactive,classified',264),
('connector_browser','Device Browser','Use an allowlisted browser on the local device for URL opening and screenshots.','POST','/connector/{device_id}/browser','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["list","open_url","screenshot"]},"browser_alias":{"type":"string"},"url":{"type":"string","format":"uri"},"scale":{"type":"number","minimum":0.1,"maximum":1.0},"timeout_ms":{"type":"integer","minimum":1000,"maximum":120000},"user_id":{"type":"string"}}}',NULL,'device,browser,interactive,classified',266),
('connector_ps','Device PowerShell','Admin-only workaround surface for recovery scripts when narrower tools cannot complete the task.','POST','/connector/{device_id}/ps','["device_id"]','{"type":"object","required":["device_id","script"],"properties":{"device_id":{"type":"string"},"script":{"type":"string","maxLength":10000},"timeout_ms":{"type":"integer","minimum":1000,"maximum":600000},"user_id":{"type":"string"}}}',NULL,'device,powershell,state_changing,admin_only,workaround',267),
('connector_win','Device Windows Control','Admin-only Windows workaround surface for screenshots, services, processes, notifications, and controlled UI recovery.','POST','/connector/{device_id}/win','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["open_url","open_vscode","screenshot","process_list","process_kill","notify","service_list","service_action","disk_list","dir_list","file_search"]},"url":{"type":"string"},"path":{"type":"string"},"pattern":{"type":"string"},"recurse":{"type":"boolean"},"limit":{"type":"integer","maximum":200},"scale":{"type":"number","minimum":0.1,"maximum":1.0},"filter":{"type":"string"},"top":{"type":"integer"},"name":{"type":"string"},"pid":{"type":"integer"},"title":{"type":"string"},"message":{"type":"string"},"service_name":{"type":"string"},"service_action":{"type":"string","enum":["start","stop","restart"]},"user_id":{"type":"string"}}}',NULL,'device,windows_control,interactive,state_changing,admin_only,workaround',268),
('connector_n8n','Device n8n Control','Admin-only local n8n workaround for workflow inspection, activation, deactivation, execution, and history recovery.','POST','/connector/{device_id}/n8n','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["health","list_workflows","get_workflow","activate_workflow","deactivate_workflow","run_workflow","list_executions"]},"workflow_id":{"type":"string"},"input_data":{"type":"object","additionalProperties":true},"limit":{"type":"integer"},"user_id":{"type":"string"}}}',NULL,'device,n8n,state_changing,admin_only,workaround',269),
('connector_cf','Device Cloudflare Control','Admin-only Cloudflare workaround for tunnel, DNS, zone, and cache recovery when primary paths are degraded.','POST','/connector/{device_id}/cf','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["list_zones","list_dns","create_dns","delete_dns","list_tunnels","tunnel_status","purge_cache"]},"zone_id":{"type":"string"},"record_id":{"type":"string"},"tunnel_id":{"type":"string"},"type":{"type":"string"},"name":{"type":"string"},"content":{"type":"string"},"proxied":{"type":"boolean"},"files":{"type":"array","items":{"type":"string"}},"user_id":{"type":"string"}}}',NULL,'device,cloudflare,dns,state_changing,admin_only,workaround',270),
('connector_fetch_upload','Device Fetch-Upload','Device fetches a URL and pushes content to platform uploads. Returns upload_id.','POST','/connector/{device_id}/fetch-upload','["device_id"]','{"type":"object","required":["device_id","url","upload_type"],"properties":{"device_id":{"type":"string"},"url":{"type":"string"},"upload_type":{"type":"string","enum":["schema","skill","knowledge","repo_link","asset"]},"filename":{"type":"string"},"user_id":{"type":"string"}}}',NULL,'device',272),
...
('admin_scope_grant_create','Create Admin Scope Grant','Admin issues a time-bounded scope grant letting a tenant user invoke a normally admin-only DB tool through /gpt/tools/call.','POST','/admin/scope-grants','[]','{"type":"object","required":["tenant_id","user_id","source_tool_key"],"properties":{"tenant_id":{"type":"string","format":"uuid"},"user_id":{"type":"string","format":"uuid"},"source_tool_key":{"type":"string"},"allowed_actions":{"type":"array","items":{"type":"string"}},"allowed_args":{"type":"object","additionalProperties":true},"reason":{"type":"string"},"expires_at":{"type":"string","format":"date-time"}}}',NULL,'admin,scope_grant,state_changing',510),
('admin_scope_grant_list','List Admin Scope Grants','Admin lists scope grants. Filter by tenant_id, user_id, source_tool_key, or active_only.','GET','/admin/scope-grants','[]','{"type":"object","properties":{"tenant_id":{"type":"string"},"user_id":{"type":"string"},"source_tool_key":{"type":"string"},"active_only":{"type":"boolean","default":true},"limit":{"type":"integer"}}}',NULL,'admin,scope_grant',511),
('admin_scope_grant_revoke','Revoke Admin Scope Grant','Admin revokes an active scope grant. Sets revoked_at and revoked_by, never deletes the row.','DELETE','/admin/scope-grants/{grant_id}','["grant_id"]','{"type":"object","required":["grant_id"],"properties":{"grant_id":{"type":"string","format":"uuid"},"reason":{"type":"string"}}}',NULL,'admin,scope_grant,state_changing',512)
ON DUPLICATE KEY UPDATE display_name=VALUES(display_name), description=VALUES(description), http_method=VALUES(http_method), http_path=VALUES(http_path), path_param_keys=VALUES(path_param_keys), input_schema=VALUES(input_schema), fixed_body=VALUES(fixed_body), tags=VALUES(tags), sort_order=VALUES(sort_order)
'@
...
('connector_files','Device Files','Explore allowlisted device paths, list drives, locate repo roots, and read or write allowlisted files.','POST','/connector/{device_id}/files','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["list","list_drives","locate_repo","read","write"]},"path":{"type":"string"},"content":{"type":"string"},"markers":{"type":"array","items":{"type":"string"}},"max_depth":{"type":"integer","minimum":0,"maximum":8},"max_matches":{"type":"integer","minimum":1,"maximum":50},"max_entries":{"type":"integer","minimum":1,"maximum":500}}}',NULL,'device,filesystem,repo_discovery',150),
('connector_apps','Device Apps','List, launch, inspect, or close allowlisted desktop applications on the local device.','POST','/connector/{device_id}/apps','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["status","list","launch","status_app","close"]},"app_alias":{"type":"string"},"timeout_ms":{"type":"integer","minimum":1000,"maximum":120000}}}',NULL,'device,desktop_app,interactive,classified',155),
('connector_browser','Device Browser','Use an allowlisted browser on the local device for URL opening and screenshots.','POST','/connector/{device_id}/browser','["device_id"]','{"type":"object","required":["device_id","action"],"properties":{"device_id":{"type":"string"},"action":{"type":"string","enum":["list","open_url","screenshot"]},"browser_alias":{"type":"string"},"url":{"type":"string","format":"uri"},"scale":{"type":"number","minimum":0.1,"maximum":1.0},"timeout_ms":{"type":"integer","minimum":1000,"maximum":120000}}}',NULL,'device,browser,interactive,classified',158),
...
('me_scope_grants_list','My Scope Grants','Tenant user reads scope grants issued to them. Returns only active, non-expired, non-revoked grants by default.','GET','/me/scope-grants','[]','{"type":"object","properties":{"active_only":{"type":"boolean","default":true}}}',NULL,'tenant,scope_grant,read_only',410)
ON DUPLICATE KEY UPDATE display_name=VALUES(display_name), description=VALUES(description), http_method=VALUES(http_method), http_path=VALUES(http_path), path_param_keys=VALUES(path_param_keys), input_schema=VALUES(input_schema), fixed_body=VALUES(fixed_body), tags=VALUES(tags), sort_order=VALUES(sort_order)
'@
